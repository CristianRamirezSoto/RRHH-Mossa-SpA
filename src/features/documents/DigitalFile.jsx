...
const emptyForm = { category: 'Curriculum vitae', expiryDate: '', observations: '', visibleToWorker: true };
...
function EmployeeFile({ employee, documents, canManage, currentUser, onBack }) {
  ...
  async function handleUpload(event) {
    ...
    try {
      ...
      const documentPayload = {
        ...
        visibleToWorker: form.visibleToWorker,
      };
      let legacySchema = false;
      try {
        await insertRow('documents', documentPayload);
      } catch (error) {
        if (!/visible_to_worker|schema cache/i.test(error.message)) throw error;
        const { visibleToWorker: _visibility, ...compatiblePayload } = documentPayload;
        await insertRow('documents', compatiblePayload);
        legacySchema = true;
      }
      setShowUpload(false);
      setForm(emptyForm);
      setFile(null);
      setMessage(legacySchema
        ? 'Documento cargado. Ejecuta supabase/sql/02_employee_portal.sql para habilitar el control de visibilidad.'
  ...
  async function toggleWorkerVisibility(item) {
    try {
      await updateRow('documents', item.id, { visibleToWorker: item.visibleToWorker === false });
      setMessage(item.visibleToWorker === false
        ? 'El documento ahora es visible para el trabajador.'
        : 'El documento quedó oculto para el trabajador.');
    } catch (error) {
      const setupHint = /visible_to_worker|schema cache/i.test(error.message)
        ? ' Ejecuta supabase/sql/02_employee_portal.sql en Supabase.'
        : '';
      setMessage(`No se pudo cambiar la visibilidad: ${error.message}.${setupHint}`);
    }
  }
    ...
    <div className="page">
      ...
      <section className="file-ops-grid">
        ...
        <article className="panel file-storage-card">
          ...
          <div>
            <strong>{canManage ? 'Acceso documental protegido' : 'Tus documentos están protegidos'}</strong>
            <p>{canManage
              ? 'Los archivos se entregan mediante enlaces temporales y la visibilidad para cada trabajador se controla individualmente.'
              : 'Solo tú y RRHH pueden acceder. Cada descarga utiliza un enlace temporal y seguro.'}</p>
          </div>
        </article>
      </section>
            ...
            <article className="document-card" key={item.id}>
              ...
              <div className="document-card-main">
                <div className="document-card-heading">
                  ...
                  {canManage && (
                    <span className={`worker-visibility-pill ${item.visibleToWorker === false ? 'hidden' : ''}`}>
                      {item.visibleToWorker === false ? 'Oculto al trabajador' : 'Visible al trabajador'}
                    </span>
                  )}
                </div>
          ...
          <form className="modal document-modal" onSubmit={handleUpload}>
            ...
            <div className="form-grid">
              ...
              <label className="consent-field field-wide">
                <input
                  type="checkbox"
                  checked={form.visibleToWorker}
                  onChange={(event) => setForm((current) => ({ ...current, visibleToWorker: event.target.checked }))}
                />
                <span>
                  <strong>Visible para el trabajador</strong>
                  <small>Podrá consultar y descargar este documento desde su portal personal.</small>
                </span>
              </label>
