...
export const documentTypes = [
  { id: 'Contrato', label: 'Contrato', required: true },
  { id: 'Anexos', label: 'Anexos', required: false },
  { id: 'Curriculum', label: 'Curriculum', required: true },
  { id: 'Carnet', label: 'Carnet', required: true },
  { id: 'Certificados', label: 'Certificados', required: true },
  { id: 'Licencias', label: 'Licencias', required: false },
  { id: 'Otros', label: 'Otros', required: false },
];

const emptyForm = { category: 'Contrato', expiryDate: '', observations: '' };
...
function EmployeeFile({ employee, documents, canManage, currentUser, onBack }) {
  ...
  async function handleUpload(event) {
    ...
    if (file.size > 15 * 1024 * 1024) {
      setMessage('El archivo no puede superar los 15 MB.');
      return;
    }
  ...
  return (
    <div className="page">
      ...
      <header className="employee-file-header">
        ...
        {canManage && <button className="primary-button" type="button" onClick={() => setShowUpload(true)}><Icon name="upload" /> Subir documento</button>}
      </header>
      ...
      <div className="category-tabs file-categories">
        {['Todos', ...documentTypes.map((item) => item.id)].map((item) => (
          <button key={item} type="button" className={category === item ? 'active' : ''} onClick={() => setCategory(item)}>
            {item}
            {item !== 'Todos' && <span>{documents.filter((docItem) => docItem.category === item).length}</span>}
          </button>
        ))}
      </div>
        ...
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setShowUpload(false)}>
          <form className="modal document-modal" onSubmit={handleUpload}>
            ...
            <div className="form-grid">
              <label className="field"><span>Tipo de documento</span><select value={form.category} onChange={(event) => setForm((current) => ({ ...current, category: event.target.value }))}>{documentTypes.map((item) => <option key={item.id}>{item.label}</option>)}</select></label>
              ...
              <label className="upload-zone field-wide" onClick={() => fileInput.current?.click()}>
                ...
                <span>{file ? formatBytes(file.size) : 'PDF, imágenes o documentos · Máximo 15 MB'}</span>
              </label>
            </div>
...
function documentSummary(documents) {
  const presentRequired = new Set(documents.filter((item) => documentTypes.find((type) => type.id === item.category)?.required).map((item) => item.category));
  ...
  return {
    ...
    alerts: states.filter((item) => ['expired', 'soon'].includes(item.type)).length,
  };
}
